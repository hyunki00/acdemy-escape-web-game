...
/* ===========================================================
   ...
   room.background: 이미지 경로, room.hotspots: 클릭 가능한 영역(%좌표) 배열
=========================================================== */
function renderImageScene(room, state){
  const hotspotsHtml = (room.hotspots || []).map(h => {
    let solved = false;
    if (h.kind === 'puzzle') solved = !!state.solved[h.id];
    if (h.kind === 'lock') solved = !!state.unlocked[h.id];
    return `<div class="hitbox ${solved ? 'is-solved' : ''}"
      style="left:${h.x}%; top:${h.y}%; width:${h.w}%; height:${h.h}%;"
      data-kind="${h.kind}" data-id="${h.id || ''}" data-dest="${h.dest || ''}"
      title="${h.label || ''}"></div>`;
  }).join('');
